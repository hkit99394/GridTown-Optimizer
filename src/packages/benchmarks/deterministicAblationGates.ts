...
export interface DeterministicAblationGateVariantSummaryInput {
  variantName: string;
  caseCount: number;
  seedCount: number;
  comparisonCount: number;
...
export interface DeterministicAblationGateSuiteInput {
  caseCount: number;
  seedCount: number;
  comparisonCount: number;
  ...
  variantSummaries: readonly DeterministicAblationGateVariantSummaryInput[];
}

export interface DeterministicAblationGateReportInput {
  greedy?: DeterministicAblationGateSuiteInput;
  lns?: DeterministicAblationGateSuiteInput;
}

export const DEFAULT_DETERMINISTIC_ABLATION_GATE_SEEDS = Object.freeze([7, 19, 37]);

function isLnsVariantSummary(summary: DeterministicAblationGateVariantSummaryInput): boolean {
  return "firstWindowMovementRate" in summary;
}

function evidenceFromSummary(summary: DeterministicAblationGateVariantSummaryInput): DeterministicAblationGateEvidence {
...
function hasIsolatedPopulationWin(summary: DeterministicAblationGateVariantSummaryInput): boolean {
...
function decideVariant(
  suite: DeterministicAblationGateSuiteName,
  summary: DeterministicAblationGateVariantSummaryInput
  ...
  if (summary.variantName === "baseline") {
    ...
    reasons.push(
      `Window choice changed without population lift: first-window-move-rate=${formatRate(summary.firstWindowMovementRate ?? 0)}, window-sequence-move-rate=${formatRate(summary.windowSequenceMovementRate ?? 0)}, anchor-coordinate-move-rate=${formatRate(summary.anchorCoordinateMovementRate ?? 0)}.`
    );
  } else {
    decision = "keep-baseline";
...
function buildSuiteReport(
  suite: DeterministicAblationGateSuiteName,
  result: DeterministicAblationGateSuiteInput
