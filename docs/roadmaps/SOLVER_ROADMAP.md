...
- Train and baseline-check the final-lift ranker on `strict-lns-replay-labels-2026-05-04-curated-seeds-v3-roll-forward-1x0-1`, using weak-seed exclusion as the default expectation rather than an after-the-fact diagnostic.
- Investigate the offline-to-online gap before any feature flag: prior weak-seed final-lift labels improved offline holdout while protected override-active online thresholds stayed neutral and fallback-only runs drifted with timing. Next evidence should test whether the full-family curated non-weak labels change the final-lift ranker’s protected online behavior, then isolate any remaining neutral transitions with richer final-state features or longer/chunked roll-forward budgets.
