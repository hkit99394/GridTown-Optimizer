...
| 13 | Feature-flagged LNS learned online A/B | delivered / ready-for-default-review | 3.0 | Phase 13 wires the Phase 12 LNS window scorer behind `lns.learnedWindowRanking`, adds guarded candidate re-ranking, validation, telemetry counters, an online A/B CLI, and strict registry entry `lns-learned-online-ab-2026-05-17`. | Guarded mode produced `18/18` protected holdout ties, zero losses, and a `0.0389s` mean wall-clock win with no default change; exploratory mode found `3` development wins and zero holdout losses. |
...
| Phase 13 LNS online A/B is ready-for-default-review but has no quality lift | LNS learned default promotion review | 2.0 | Decide whether a guarded wall-clock-only win is enough for an opt-in-to-default change, or require a follow-up with population/time-to-quality lift. | Either explicitly keep `lns.learnedWindowRanking` opt-in, or promote with a documented rollback gate and a broader product-workflow scorecard. |
