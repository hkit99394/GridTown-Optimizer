...
- Offline-to-online gap diagnostics now exist behind `lnsWindowRankerCli --gap-diagnostics`, joining trained-model replay decisions from a label artifact to protected online scorecard override transitions without rerunning CP-SAT or LNS. The registered diagnostic draft `lns-window-ranker-gap-2026-05-05-curated-seeds-v3-final-lift-no-weak-protected` is generated under `artifacts/lns-window-ranker-gap-diagnostics/2026-05-05/strict-lns-replay-labels-curated-seeds-v3-final-lift-no-weak-protected-gap/` with dry-run registry validation only and regenerated online scorecard fingerprint `fnv1a:17d43053`. It confirms the blocker has two concrete shapes: 30/30 online overrides are final-neutral; 3 protected transition/family pairs are offline holdout-positive but online final-neutral (`corridor/random-exploration->random-exploration`, `footprint-pressure/weak-service->sliding`, and `gate/weak-service->sliding`); and 6 protected transition/family pairs are online-active with no matching offline holdout transition. The diagnostic now records `selection-trace=30`, and promotion remains blocked.

Concrete work:

- Use the per-decision online trace rows to compare the three offline-positive/online-neutral transition families at selected-window, feature-delta, and final-layout level; the next diagnostic should explain whether the miss is geometry mismatch, feature over-weighting, or insufficient roll-forward depth.
- Patch the offline evidence gap before retraining: either add targeted protected-holdout replay labels for online-active/no-offline-match transition families or gate those unmatched transitions out of promotion diagnostics until matching holdout replay evidence exists.
- Add richer final-state or trajectory features only where the joined diagnostic explains the miss: likely candidates are final placement deltas, accepted-improvement depth after the selected repair, unchanged-population layout-distance metrics, or longer/chunked roll-forward budgets that separate genuine final-lift opportunities from harmless intermediate reshuffles.
