...
3. **Fresh manual-resume holdout:** done on 2026-06-01. `fresh-manual-resume-neighborhood` is now in the product corpus with a replay hint, LNS seed hint, CP-SAT warm-start hint, and the `manual-resume-neighborhood` workflow tag. Run a focused refresh before any promotion claim around saved-layout reuse, warm-start behavior, or planner resume flows.
...
Full split-baseline coverage on 2026-05-31: split-lane artifacts cover the previous 14 product-corpus cases, all four modes, budgets `1,5,30,120`, and seeds `7,19,37`: 168 scorecards and 672 mode runs. Auto tied best on 163 of 168 rows; the five gaps are short-budget rows only. Auto tied best on all `30s` and `120s` rows. Decision: keep the split artifacts as the durable baseline. Create one combined promotion-matrix artifact only if a release process explicitly requires it; refresh `fresh-manual-resume-neighborhood` first for current 15-case baseline claims.
...
- **Fresh manual-resume holdout:** completed 2026-06-01. `fresh-manual-resume-neighborhood` adds a fresh saved-layout resume and warm-start holdout row; focused evaluator-validity and baseline refresh should precede planner-resume promotion claims.
