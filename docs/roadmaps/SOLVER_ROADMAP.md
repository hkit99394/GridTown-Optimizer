...
- Greedy offline ranking has a protected-holdout diagnostic win and a feature-flagged runtime hook, but its first equal-budget online A/B did not earn promotion; enriched LNS offline ranking beat offline baselines, but online review and guard calibration found no product-holdout quality lift, so it also remains opt-in.
...
| 12 | Enriched LNS ranker feature gate | delivered / fed Phase 13 | 3.0 | Phase 12 enriches LNS replay/window features with operator identity, candidate rank, anchor/boundary pressure, allowed/empty-cell counts, footprint overlap, density signals, regenerated split-protected labels, and strict registry entry `lns-ranker-feature-enrichment-2026-05-17`. | Protected holdout accuracy reaches `100.0%`, beating deterministic window proxy `97.9%`, random `50.7%`, and best single-feature `97.9%`; the scorer is now integrated behind the Phase 13 feature flag. |
| 13 | Feature-flagged LNS learned online A/B | delivered / fed Phase 14 | 3.0 | Phase 13 wires the Phase 12 LNS window scorer behind `lns.learnedWindowRanking`, adds guarded candidate re-ranking, validation, telemetry counters, an online A/B CLI, and strict registry entry `lns-learned-online-ab-2026-05-17`. | Guarded mode produced `18/18` protected holdout ties, zero losses, and a `0.0389s` mean wall-clock win with no default change; exploratory mode found `3` development wins and zero holdout losses. |
| 14 | LNS learned default promotion review | delivered / no-promotion | 2.0 | Phase 14 adds a broader product-workflow plus cross-mode promotion-review scorecard for `lns.learnedWindowRanking`, exact solution validation, CLI artifact output, and strict registry entry `lns-learned-promotion-review-2026-05-17`. | Product holdout had `4/4` ties, zero losses, zero validation failures, but no quality lift and a `0.325s` mean wall regression, so `lns.learnedWindowRanking` remains opt-in. |
| 15 | LNS learned guard calibration | delivered / no-promotion | 1.5 | Phase 15 parameterizes the guarded min-score ratio, adds a calibration sweep CLI, tests the evidence wrapper, and records strict registry entry `lns-learned-guard-calibration-2026-05-17`. | Ratios `1`, `0.95`, `0.9`, `0.75`, and `0` were all regression-safe over `60` paired comparisons, but `0/5` ratios produced product-holdout quality lift, so no promotion retry is recommended. |
| 16 | LNS learned displacement diagnostics | delivered / objective-gated | 1.5 | Phase 16 adds learned-displacement telemetry, compares strict guard, relaxed guard, and widened shortlist configurations, and records strict registry entry `lns-learned-displacement-diagnostics-2026-05-17`. | All `3/3` configurations were regression-safe, but `0/3` produced product-holdout quality lift; relaxed and widened configs displaced windows without final lift, so retrain or retarget the LNS window objective before another promotion review. |
...
| Enriched LNS ranker feature gate | delivered / fed Phase 13 | `artifacts/lns-ranker-feature-enrichment/2026-05-17/lns-ranker-feature-enrichment.json`; registry run `lns-ranker-feature-enrichment-2026-05-17`; targeted ranker tests | Enriched CPU-only LNS ranker beats deterministic/random/single-feature offline holdout baselines and is now embedded behind the Phase 13 `lns.learnedWindowRanking` flag. |
| LNS learned online A/B | delivered / fed Phase 14 | `artifacts/lns-online-ab/2026-05-17/lns-online-ab.json`; registry run `lns-learned-online-ab-2026-05-17`; targeted online-A/B tests | `lns.learnedWindowRanking` exists as an opt-in flag. Guarded mode has zero protected holdout losses and a small mean wall-clock win, but no quality lift; no LNS default changed. |
| LNS learned promotion review | delivered / no-promotion | `artifacts/lns-promotion-review/2026-05-17/lns-promotion-review.json`; registry run `lns-learned-promotion-review-2026-05-17`; targeted promotion-review tests | Broader product/cross-mode review produced zero regressions and zero validation failures, but no product-holdout quality lift; `lns.learnedWindowRanking` remains opt-in. |
| LNS learned guard calibration | delivered / no-promotion | `artifacts/lns-guard-calibration/2026-05-17/lns-guard-calibration.json`; registry run `lns-learned-guard-calibration-2026-05-17`; targeted calibration tests | Loosening `lns.learnedWindowRankingMinScoreRatio` to `0.95`, `0.9`, `0.75`, or `0` stayed regression-safe but still produced no product-holdout quality lift; no promotion retry is recommended. |
| LNS learned displacement diagnostics | delivered / objective-gated | `artifacts/lns-displacement-diagnostics/2026-05-17/lns-displacement-diagnostics.json`; registry run `lns-learned-displacement-diagnostics-2026-05-17`; targeted diagnostics tests | Relaxed and widened learned configs displace windows, but those displaced windows do not create final product-holdout quality lift; the blocker is the LNS window objective/labels, not just guard strictness or shortlist size. |
| Model training path | partial / gated | Phase 9 has an offline Greedy ranker metric report, Phase 10 has a feature-flagged Greedy scorer, Phase 12 has an enriched offline LNS ranker report, and Phases 13-16 have a feature-flagged LNS scorer plus no-promotion review, guard calibration, and displacement diagnostics; no `python/ml/` scaffold, runtime model loading path, or learned default is promoted | Learned scorers exist only as explicit flags; no learned default path. |
...
| New LNS scorer or window objective beats the Phase 16 displacement baseline | LNS learned promotion retry | 2.0 | Retrain or retarget learned window scoring toward final repair lift before another default-promotion review; guard and shortlist changes alone did not create lift. | Product/cross-mode review has zero validation failures, zero protected-holdout losses, no worst-decile regression, and a real quality or time-to-quality win. |
...
5. Keep the Phase 12/13 LNS ranker feature-flagged; Phases 15-16 showed guard loosening and shortlist widening are safe but not useful, so do not change LNS defaults until a new scorer or objective improves fixed-budget quality or time-to-best.
...
7. Revisit learned defaults only after offline holdout, equal-budget online gates, and product/cross-mode promotion review pass with a real quality or time-to-best win.
