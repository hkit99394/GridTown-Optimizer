...
- LNS replay-label collection now has an opt-in curated non-weak seed path. `lnsBenchmarkCli --window-replay-labels --curated-replay-seeds` and `learnedRankingLabelCli --curated-lns-replay-seeds` use curated first-improvement replay seeds for `lns-gate-choke-pressure` and `lns-service-overlap-pressure` while leaving default weak-seed and natural-seed behavior unchanged.
...
- Broaden non-weak curated final-lift opportunity beyond the current gate/service seed coverage by adding curated incumbents or new protected replay cases for corridor, footprint, anchor-service, and service-pressure families that still miss `post-first-improvement` coverage before any learned online promotion.
