...
Review result: the stage split is still healthy. Short run is complete, middle-run framework work is complete, and long-run/default-path promotion remains intentionally gated. The current 2026-05-31 split-lane baseline is enough for normal candidate comparison; a single combined promotion-matrix artifact is unnecessary unless a release process explicitly requires it.

Findings from this review:

1. **P1 evidence hygiene:** fixed on 2026-06-01 by [ARTIFACT_HYGIENE_RECOVERY_PLAN.md](ARTIFACT_HYGIENE_RECOVERY_PLAN.md). The repo externalized and untracked 234 unindexed raw artifacts, uploaded durable raw packages to the `solver-evidence-2026-06-01` GitHub Release, and recovered the evidence gate. `npm run artifact-hygiene:inventory` now reports 1482 tracked artifact files before staging the compact external manifest.
2. **P2 stale CPU baseline wording:** fixed in [MIDDLE_RUN_CPU_TIME_TO_BEST_REVIEW.md](MIDDLE_RUN_CPU_TIME_TO_BEST_REVIEW.md). The review now uses the 2026-05-31 14-case split baseline instead of the legacy 2026-04-30 10-case bundle.
3. **P3 L0 status drift:** fixed in [L0_PRE_LONG_RUN_CHECKPOINT.md](L0_PRE_LONG_RUN_CHECKPOINT.md). L0 is now described as the active pre-long-run handoff, not only as the first CP-SAT preflight.
...
3. **Fresh manual-resume holdout:** done on 2026-06-01. `fresh-manual-resume-neighborhood` is now in the product corpus with a replay hint, LNS seed hint, CP-SAT warm-start hint, and the `manual-resume-neighborhood` workflow tag. Its focused `1s/5s` refresh is population-clean, with Auto reaching cap on all rows; finish `30s/120s` and review the `1s` Auto timing overrun before any promotion claim around saved-layout reuse, warm-start behavior, or planner resume flows.
...
Corpus coverage audit updated on 2026-06-01: [MIDDLE_RUN_CORPUS_COVERAGE_AUDIT.md](MIDDLE_RUN_CORPUS_COVERAGE_AUDIT.md) records the product-workflow split, mode, budget, seed, replay, and workflow-family coverage. The 2026-05-31 split-lane baseline covers the previous 14 product-corpus cases, all four modes, budgets `1,5,30,120`, and seeds `7,19,37`; the current 15-case corpus now has `fresh-manual-resume-neighborhood` refreshed at `1s/5s`, with `30s/120s` pending before current-corpus full-baseline claims. Candidate-specific evaluator-validity evidence is still required before promotion claims.
...
Baseline scorecard refresh plan updated on 2026-06-01: [MIDDLE_RUN_BASELINE_SCORECARD_REFRESH_PLAN.md](MIDDLE_RUN_BASELINE_SCORECARD_REFRESH_PLAN.md) names smoke, development, protected holdout, fresh holdout, and full promotion-matrix refresh commands plus storage conventions. The fresh product holdout set, the expansion/corridor development analog, and the manual-resume fresh row are now implemented; a new full promotion refresh should expect 15 product workflow cases.
...
Full split-baseline coverage on 2026-05-31 plus manual-resume fast lane on 2026-06-01: split-lane artifacts cover the previous 14 product-corpus cases, all four modes, budgets `1,5,30,120`, and seeds `7,19,37`, and the new manual-resume row at `1s/5s`: 174 scorecards and 696 mode runs. The full current 15-case baseline still needs the manual-resume `30s/120s` lanes to reach 180 scorecards and 720 mode runs. Decision: keep the split artifacts as the durable baseline. Create one combined promotion-matrix artifact only if a release process explicitly requires it.
...
- **Fresh holdout `120s` baseline refresh:** completed 2026-05-31. The first three-case fresh holdout split passed the `120s`, seeds `7,19,37` refresh with Auto tied best on all 9 rows; the 2026-05-31 split-lane baseline covers the previous 14 cases, all four modes, all four standard budgets, and all three standard seeds.
- **Fresh manual-resume holdout:** completed 2026-06-01. `fresh-manual-resume-neighborhood` adds a fresh saved-layout resume and warm-start holdout row; its focused evaluator-validity and `1s/5s` baseline refresh are population-clean, with `30s/120s` and `1s` timing-overrun review still pending before planner-resume promotion claims.
