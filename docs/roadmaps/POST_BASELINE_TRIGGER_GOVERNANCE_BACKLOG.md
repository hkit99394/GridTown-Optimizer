...
1. **G4 checker edge audit.** Add only targeted tests that protect real review mistakes.
2. **G5 artifact soft-warning operating rule.** Clarify what reviewers should do while the repo sits one file above the soft target.
3. **G6 PR review response runbook.** Prepare for review comments without mixing unrelated fixes.
4. **G7-G8 only on trigger.** Do not start solver implementation from this backlog unless the trigger ledger admits a current signal.
