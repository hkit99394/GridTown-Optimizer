...
3. **Fresh manual-resume holdout:** done on 2026-06-01. `fresh-manual-resume-neighborhood` is now in the product corpus with a replay hint, LNS seed hint, CP-SAT warm-start hint, and the `manual-resume-neighborhood` workflow tag. Its focused `1s/5s/30s/120s` split-lane refresh is population-clean, with Auto reaching the hard cap on every row. Review the `1s` Auto timing overrun before any promotion claim around saved-layout reuse, warm-start behavior, or planner resume flows.
...
- **Fresh manual-resume holdout:** completed 2026-06-01. `fresh-manual-resume-neighborhood` adds a fresh saved-layout resume and warm-start holdout row; its focused evaluator-validity and `1s/5s/30s/120s` baseline refresh are population-clean. The remaining maintenance item is `1s` Auto timing-overrun review before planner-resume promotion claims.
