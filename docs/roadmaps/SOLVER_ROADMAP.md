...
| --- | --- | --- | ---: | --- | --- |
| 1 | Solver telemetry manifests | partial | 4.0 | Cross-mode scorecard artifact runs now emit `telemetry-manifest.json` with command, git, hardware, per-run solver params, wall/CPU timing, first-feasible and best-score timing, status/gap, candidate-count, CP-SAT model-size, and per-stage Auto/Greedy/LNS/CP-SAT records; product-corpus bundles also carry evidence and registry drafts. Remaining work is to extend the manifest contract into workflow, label, and model artifact paths. | Every benchmark and workflow run can explain where time was spent and why a candidate change did or did not improve. |
