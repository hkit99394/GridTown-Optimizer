...
- road-semantics scorecard closeout with per-component anchor connectivity
- label/replay support used by the LNS replay scale-up and learned-window diagnostics
...
### 2. Maintain CP-SAT as a label and replay engine

Impact on target: high enabling value

Why it matters:

- learned ranking and better `LNS` control depend on trustworthy labels for "what would have happened if we repaired this different window?"
...
- Phase 8 uses CP-SAT repair to produce split-protected pairwise `LNS` replay labels across five pressure families per split.
- Phases 11-16 use those labels and later replay diagnostics to train, integrate, review, calibrate, and reject default promotion for the opt-in learned LNS window scorer.
- Exact validation remains through the existing evaluator.

Future scope:

- extend replay only when a new scorer, window objective, or repair-budget coupling is proposed
- preserve seed-quality comparisons for warm starts and objective lower bounds
- keep exact upper-bound and gap export available in shared traces
- keep benchmark-safe replay artifacts tied to evaluator-valid solution shapes
