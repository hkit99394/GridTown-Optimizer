...
- Offline-to-online gap diagnostics now exist behind `lnsWindowRankerCli --gap-diagnostics`, joining trained-model replay decisions from a label artifact to protected online scorecard override transitions without rerunning CP-SAT or LNS. The registered diagnostic draft `lns-window-ranker-gap-2026-05-05-curated-seeds-v3-final-lift-no-weak-protected` is generated under `artifacts/lns-window-ranker-gap-diagnostics/2026-05-05/strict-lns-replay-labels-curated-seeds-v3-final-lift-no-weak-protected-gap/` with dry-run registry validation only. It confirms the blocker has two concrete shapes: 30/30 online overrides are final-neutral; 3 protected transition/family pairs are offline holdout-positive but online final-neutral (`corridor/random-exploration->random-exploration`, `footprint-pressure/weak-service->sliding`, and `gate/weak-service->sliding`); and 6 protected transition/family pairs are online-active with no matching offline holdout transition. Promotion remains blocked.

Concrete work:

- Add per-decision online trace export for ranker selections so the three offline-positive/online-neutral transition families can be compared at selected-window and final-layout level, not just aggregate transition level.
