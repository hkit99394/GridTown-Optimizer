...
- Protected threshold sweeps are now allowed in the online ranker CLI, so calibration can be run directly against the independent holdout corpus. An ad hoc protected sweep for the tuned final-lift model over `minScoreDelta=0.1,0.25,0.5,0.75,1,1.5` found no protected lift at any threshold. At `0.5`, overrides dropped to 15/30 with 15 fallbacks and still ended 0/15 improved, 0/15 regressed, 15/15 unchanged; `0.75+` fell back on all 30 decisions and also stayed neutral. A single follow-up `0.25` scorecard was neutral even though the sweep observed one timing-sensitive `-45` service-ridge regression, so thresholding alone is not a promotion path.
...
- Investigate the offline-to-online gap before any feature flag: the final-lift labels and tuned ranker improve offline holdout, but protected online remains neutral and thresholding only changes override volume. Next evidence should artifact protected threshold sweeps, inspect selected-vs-baseline transition diagnostics on real scorecards, and then test richer final-state features or longer/chunked roll-forward budgets.
