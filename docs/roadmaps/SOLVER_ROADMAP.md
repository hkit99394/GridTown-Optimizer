...
| --- | --- | --- | ---: | --- | --- |
| 1 | Product-shaped benchmark corpus | partial | 4.5 | Initial `--product-corpus` cross-mode selector now covers solver smoke, manual-layout replay, expansion-comparison replay, corridor/gate/footprint/service-pressure, anchor-service, and multi-anchor cases with dev/holdout metadata. Artifact writing, registry drafts, strict registry dry-run support, `--product-promotion-matrix`, evidence summaries, per-case metrics, and initial `/api/layout/evaluate` replay metrics are available. A non-promotion 1s/5s seed-7 artifact is registered at `artifacts/product-corpus/2026-04-30/initial-1s-5s-seed7-v2/`; it is intentionally partial because 30s/120s budgets and additional seeds are missing. Remaining work is executing promotion-grade multi-budget dev/holdout artifacts, checkpointing those artifacts, and appending strict registry entries with truthful artifact commits. | Promotion decisions use fixed-seed dev/holdout corpora with 1s/5s/30s/120s budgets and strict registry entries. |
...
| Product-shaped benchmark corpus | partial | `node dist/crossModeBenchmarkCli.js --product-corpus --list` exposes 10 dev/holdout cases with workflow tags; `--product-promotion-matrix` pins the long-run matrix to `auto,greedy,lns,cp-sat`, budgets `1,5,30,120`, and seeds `7,19,37`; `--product-artifact-dir` writes scorecard, evidence summary, and registry-entry draft artifacts; `--product-register-dry-run` completes strict validation without appending; direct generated-artifact append is blocked until artifacts are checkpointed; `product-corpus-scorecard-2026-04-30-initial-1s-5s-seed7-v2` is registered as initial non-promotion evidence; protected holdout status now requires full case, expected split, mode, budget, seed, and per-scorecard mode coverage | Ready for fixed-seed artifact runs; promotion-grade entries still need to be captured, checkpointed, and appended. |
...
| CP-SAT async/portfolio failure-mode coverage | partial | OR-Tools-free regressions cover malformed progress, child exit diagnostics, and process-pool fallback paths | Continue before raising fan-out limits or distributed orchestration. |
| Adaptive LNS | active | Current LNS has ranked windows and replay labels but not operator scoring | Main next quality engine after model alignment, product corpus, and telemetry. |
