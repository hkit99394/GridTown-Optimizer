...
export type LnsLearnedOnlineVariantName =
  | "baseline"
  | "learned-guarded"
  | "learned-exploratory";

export interface LnsLearnedOnlineAbRunOptions {
...
export interface LnsLearnedOnlineAbVariantResult {
  variantName: LnsLearnedOnlineVariantName;
...
export interface LnsLearnedOnlineAbComparison {
  variantName: Exclude<LnsLearnedOnlineVariantName, "baseline">;
  populationDeltaVsBaseline: number;
  wallClockDeltaVsBaselineSeconds: number;
}

export interface LnsLearnedOnlineAbCaseResult {
  split: LearnedRankingLabelSplit;
  caseName: string;
  seed: number;
  variants: LnsLearnedOnlineAbVariantResult[];
  comparisons: LnsLearnedOnlineAbComparison[];
}

export interface LnsLearnedOnlineAbSummary {
  variantName: Exclude<LnsLearnedOnlineVariantName, "baseline">;
...
export interface LnsLearnedOnlineAbGate {
  passed: boolean;
  failedReasons: string[];
  protectedHoldout: boolean;
  promotedVariant: "learned-guarded";
  holdout: LnsLearnedOnlineAbSummary;
}

export interface LnsLearnedOnlineAbResult {
  generatedAt: string;
  schemaVersion: 1;
  seeds: number[];
  selectedCaseNames: string[];
  variants: LnsLearnedOnlineVariantName[];
  ...
  cases: LnsLearnedOnlineAbCaseResult[];
  summaries: LnsLearnedOnlineAbSummary[];
  gate: LnsLearnedOnlineAbGate;
  hardware: Record<string, unknown> & {
    captured: boolean;
    gpuUsed: boolean;
  };
  decision: "lns-learned-window-ranking-ready-for-default-review" | "keep-learned-window-ranking-feature-flagged";
  summary: string;
}

export interface LnsLearnedOnlineAbSnapshot
  extends Omit<LnsLearnedOnlineAbResult, "generatedAt"> {}

const DEFAULT_LNS_ONLINE_AB_CANDIDATE_LIMIT = 12;
const DEFAULT_LNS_ONLINE_AB_EXPLORATORY_MIN_SCORE_RATIO = 0;
...
function assertOnlineAbOptions(options: {
  ...
  if (!Number.isInteger(options.candidateLimit) || options.candidateLimit <= 0) {
    throw new Error("Expected LNS learned online A/B candidate limit to be a positive integer.");
  }
  if (
    ...
    throw new Error("Expected LNS learned online A/B exploratory min-score ratio to be between 0 and 1.");
  }
}

function defaultOnlineAbCaseNames(): string[] {
  return DEFAULT_LEARNED_RANKING_LABEL_SPLITS.flatMap((split) => split.lnsCaseNames);
}
...
function splitCases(): Record<LearnedRankingLabelSplit, string[]> {
  ...
  return {
    development: [...development.lnsCaseNames],
    holdout: [...holdout.lnsCaseNames],
  };
}
...
function selectedCases(
  ...
  return selectBenchmarkCasesByName(corpus, names, {
    caseLabel: "LNS learned online A/B",
    corpusLabel: "LNS learned online A/B",
  });
}
...
function runVariant(options: {
  ...
  candidateLimit: number;
  exploratoryMinScoreRatio: number;
}): LnsLearnedOnlineAbVariantResult {
  const { benchmarkCase, seed, variantName, lns, cpSat, greedy, candidateLimit, exploratoryMinScoreRatio } = options;
  ...
  const result = runLnsBenchmarkSuite([benchmarkCase], {
  ...
  return {
    ...
    improvingIterations: result.lnsTelemetry?.improvingIterations ?? 0,
    learnedWindowRankingEvaluations: result.lnsTelemetry?.learnedWindowRankingEvaluations ?? 0,
    learnedWindowRankingWins: result.lnsTelemetry?.learnedWindowRankingWins ?? 0,
  };
}

function buildComparisons(
  variants: readonly LnsLearnedOnlineAbVariantResult[]
): LnsLearnedOnlineAbComparison[] {
  const baseline = variants.find((variant) => variant.variantName === "baseline");
  if (!baseline) throw new Error("LNS learned online A/B result is missing baseline variant.");
  return variants
    .filter((variant): variant is LnsLearnedOnlineAbVariantResult & {
      variantName: Exclude<LnsLearnedOnlineVariantName, "baseline">;
...
function summarizeVariant(
  cases: readonly LnsLearnedOnlineAbCaseResult[],
  variantName: Exclude<LnsLearnedOnlineVariantName, "baseline">,
  split: LearnedRankingLabelSplit | "all"
): LnsLearnedOnlineAbSummary {
  ...
  return {
    ...
    learnedWindowRankingEvaluations: sumBenchmarkBy(variantRuns, (variant) => variant.learnedWindowRankingEvaluations),
    learnedWindowRankingWins: sumBenchmarkBy(variantRuns, (variant) => variant.learnedWindowRankingWins),
  };
}

function buildSummaries(cases: readonly LnsLearnedOnlineAbCaseResult[]): LnsLearnedOnlineAbSummary[] {
  const summaries: LnsLearnedOnlineAbSummary[] = [];
...
function buildGate(
  ...
  summaries: readonly LnsLearnedOnlineAbSummary[]
): LnsLearnedOnlineAbGate {
  const holdout = summaries.find((summary) =>
    summary.variantName === "learned-guarded" && summary.split === "holdout"
  );
  if (!holdout) throw new Error("LNS learned online A/B result is missing guarded holdout summary.");
...
function summarizeRun(gate: LnsLearnedOnlineAbGate, summaries: readonly LnsLearnedOnlineAbSummary[]): string {
...
export function runLnsLearnedOnlineAb(
  ...
  const seeds = normalizeBenchmarkSeeds(options.seeds, "LNS learned online A/B seeds")
    ?? [...DEFAULT_DETERMINISTIC_ABLATION_GATE_SEEDS];
  const candidateLimit = options.learnedWindowRankingCandidateLimit ?? DEFAULT_LNS_ONLINE_AB_CANDIDATE_LIMIT;
  const exploratoryMinScoreRatio =
    options.exploratoryMinScoreRatio ?? DEFAULT_LNS_ONLINE_AB_EXPLORATORY_MIN_SCORE_RATIO;
  assertOnlineAbOptions({ candidateLimit, exploratoryMinScoreRatio });
  const variantNames: LnsLearnedOnlineVariantName[] = [
    "baseline",
    "learned-guarded",
    "learned-exploratory",
  ];
  const results: LnsLearnedOnlineAbCaseResult[] = [];
  ...
  const decision = gate.passed
    ? "lns-learned-window-ranking-ready-for-default-review"
    : "keep-learned-window-ranking-feature-flagged";
...
export function createLnsLearnedOnlineAbSnapshot(
  result: LnsLearnedOnlineAbResult
): LnsLearnedOnlineAbSnapshot {
  const { generatedAt: _generatedAt, ...snapshot } = result;
  return snapshot;
}

export function formatLnsLearnedOnlineAb(result: LnsLearnedOnlineAbResult): string {
  const lines: string[] = [];
  lines.push("=== LNS Learned Window Ranking Online A/B ===");
