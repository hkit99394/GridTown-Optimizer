...
- Investigate the offline-to-online gap before any feature flag: the full-family curated non-weak final-lift labels now clear offline baselines perfectly, but protected online scorecards still finish final-neutral across every override-active threshold. Next evidence should isolate why the offline label payload chooses `random-exploration->random-exploration` and `weak-service->sliding` transitions that improve replay/roll-forward labels but do not change final protected populations.
- Add richer final-state or trajectory features only if they explain the neutral protected transitions: candidate ideas include final placement deltas, accepted-improvement depth after the selected repair, unchanged-population layout-distance metrics, or longer/chunked roll-forward budgets that separate genuine final-lift opportunities from harmless intermediate reshuffles.
