...
5. **Candidate intake only on trigger:** do not open another broad solver candidate just to keep momentum. Reopen Auto/LNS only for a reproducible outside-envelope failure, CP-SAT geometry only for a different runtime-bottleneck hypothesis, learned guidance only for protected/fresh online value coverage, and service-master only for repeatable equal-budget wins.
6. **Long-run promotion:** once a triggered candidate exists, use M9 intake, same-slice baseline repeat, candidate-specific evaluator validity, CPU/time-to-best review, artifact storage handoff, and decision closeout before changing defaults.
